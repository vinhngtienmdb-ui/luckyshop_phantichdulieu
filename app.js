...
import { getFirestore, collection, doc, setDoc, updateDoc, onSnapshot, getDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/9.22.2/firebase-firestore.js";
...
if (true) {
    ...
    const db = getFirestore(app, "ai-studio-855a4b8a-5ffb-4334-8296-6a2455e93c44");
    ...
    const loadUsersList = () => {
        ...
        usersListUnsub = onSnapshot(collection(db, 'users'), snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                const isSuperAdmin = data.email === 'vinh.ngtienmdb@gmail.com' || data.email === 'admin@admin.com';
    ...
    const toastContainer = document.getElementById('toast_container') || (function() {
    ...
    window.updateUserRole = (userId, newRole) => {
        ...
        updateDoc(doc(db, 'users', userId), { role: newRole })
            .catch(err => showToast("Lỗi cập nhật quyền: " + err.message, 'error'));
    };
                        ...
                        createUserWithEmailAndPassword(auth, email, pwd)
                            ...
                            .catch(createError => {
                                if (createError.code === 'auth/email-already-in-use') {
                                    ...
                                    loginError.innerText = "Vui lòng bật 'Email/Password' trong Authentication của Firebase Console.";
                                } else {
                                    loginError.innerText = "Đăng ký thất bại: " + createError.message;
                                }
                                loginError.style.display = 'block';
                            });
    ...
    if (btnSaveConfig) {
        btnSaveConfig.addEventListener('click', () => {
            ...
            setDoc(doc(db, 'configs', 'main'), newConfig, { merge: true })
                .then(() => showToast("Lưu cấu hình thành công! Mọi người dùng trên hệ thống sẽ tự động thấy hệ số mới.", 'success'))
